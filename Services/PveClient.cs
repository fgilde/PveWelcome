...
public class PveClient(HttpClient http, ConnectionConfig config, ILogger<PveClient> log)
{
    ...
    private async Task<string?> TryGetIpAsync(PveGuest g)
    {
        try
        {
            ...
            else
            {
                ...
                return await TryGetCloudInitIpAsync(g);
            }
        }
        catch { }
        return null;
    ...
    private async Task<string?> TryGetCloudInitIpAsync(PveGuest g)
    {
        try
        {
            var cfg = await GetDataAsync($"/nodes/{g.Node}/qemu/{g.VmId}/config");
            foreach (var prop in cfg.EnumerateObject())
            {
                if (!prop.Name.StartsWith("ipconfig")) continue;
                var val = prop.Value.GetString() ?? "";
                var m = System.Text.RegularExpressions.Regex.Match(val, @"ip=([0-9]{1,3}(?:\.[0-9]{1,3}){3})");
                if (m.Success && !m.Groups[1].Value.StartsWith("127.")) return m.Groups[1].Value;
            }
        }
        catch { }
        return null;
    ...
    /// Run a bash script inside a QEMU VM via the guest agent (PVE token, no SSH); returns (exitCode, stdout, stderr).
    public async Task<(int exit, string outp, string err)> GuestExecAsync(string node, int vmid, string script, CancellationToken ct = default)
    {
        try
        {
            var b64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(script.Replace("\r\n", "\n")));
            var post = Req(HttpMethod.Post, $"/nodes/{node}/qemu/{vmid}/agent/exec");
            post.Content = new FormUrlEncodedContent(new[]
            {
                new KeyValuePair<string, string>("command", "bash"),
                new KeyValuePair<string, string>("command", "-c"),
                new KeyValuePair<string, string>("command", $"echo {b64} | base64 -d | bash"),
            });
            using var res = await http.SendAsync(post, ct);
            if (!res.IsSuccessStatusCode)
            {
                var body = await res.Content.ReadAsStringAsync(ct);
                var hint = (int)res.StatusCode == 500 ? " (Guest-Agent nicht installiert/aktiv? Dann SSH-Modus nutzen.)" : "";
                return (-1, "", $"agent/exec {(int)res.StatusCode}: {body[..Math.Min(200, body.Length)]}{hint}");
            }
            var pidDoc = await JsonDocument.ParseAsync(await res.Content.ReadAsStreamAsync(ct), cancellationToken: ct);
            var pid = pidDoc.RootElement.GetProperty("data").GetProperty("pid").GetInt32();

            for (var i = 0; i < 1800; i++)
            {
                ct.ThrowIfCancellationRequested();
                var st = await GetDataAsync($"/nodes/{node}/qemu/{vmid}/agent/exec-status?pid={pid}");
                if (st.TryGetProperty("exited", out var e) && e.GetInt32() == 1)
                {
                    var outp = st.TryGetProperty("out-data", out var o) ? o.GetString() ?? "" : "";
                    var err = st.TryGetProperty("err-data", out var er) ? er.GetString() ?? "" : "";
                    var code = st.TryGetProperty("exitcode", out var ec) ? ec.GetInt32() : -1;
                    return (code, outp, err);
                }
                await Task.Delay(1000, ct);
            }
            return (-1, "", "Timeout beim Warten auf den Guest-Agent.");
        }
        catch (OperationCanceledException) { return (-1, "", "abgebrochen"); }
        catch (Exception ex) { log.LogWarning(ex, "guest exec {Vmid}", vmid); return (-1, "", ex.Message); }
    }
    ...
    /// DESTRUCTIVE in-place restore of a guest from a backup volume (force=1); returns null on success, else an error.
